...
LABELS_PATH = BASE_DIR / "labels_b.txt"
SOURCE_DIR = BASE_DIR / "section_b"
LINES_PATH = BASE_DIR / "section_b_lines.txt"

OUT_DIR = BASE_DIR / "section_b_xtts_dataset"
...
def main():
    ...
    assert len(labels) == 60, f"Expected 60 labels, found {len(labels)}"
    assert labels["label_num"].min() == 1, "Expected labels to start at 1"
    assert labels["label_num"].max() == 60, "Expected labels to end at 60"

    labels["source_idx"] = ((labels["label_num"] - 1) // 10) + 1

    source_files = sorted(SOURCE_DIR.glob("*.wav"))
    assert len(source_files) == 6, f"Expected 6 source wav files, found {len(source_files)}"
    ...
    assert len(lines) == 60, f"Expected 60 section B lines, found {len(lines)}"

    manifest_rows = []

    for source_idx in range(1, 7):
